// Refactored LLM Playground with decoupled message handling using event system
...
use crate::llm_playground::{
    ...
    event_system::{use_event_bus, PlaygroundEvent},
    orchestrator::use_playground_orchestrator,
...
pub fn flexible_llm_playground_refactored() -> Html {
    ...
    // Event system
    let (event_bus, emit_event) = use_event_bus();
    
    // Notification system
    let (notifications, add_notification, dismiss_notification) = use_notifications();

    // Initialize orchestrator
    let orchestrator = use_playground_orchestrator(
        sessions.clone(),
        api_config.clone(),
        (*llm_client).clone(),
        (*mcp_client).clone(),
    ...
    // Set up event listeners
    {
        ...
        use_effect_with((event_bus.clone(), emit_event.clone()), move |(_, emit)| {
            ...
            let event_callback = {
                ...
                Callback::from(move |event: PlaygroundEvent| {
                    match &event {
                        PlaygroundEvent::LoadingStateChanged { is_loading: loading, .. } => {
            ...
            // Store the callback for the orchestrator to use
            // This is a simplified approach - in production you'd use a proper event bus
    ...
    // DECOUPLED MESSAGE HANDLING - This is the key change!
    let send_message = {
        let orchestrator = orchestrator.clone();
        let current_session_id = current_session_id.clone();
