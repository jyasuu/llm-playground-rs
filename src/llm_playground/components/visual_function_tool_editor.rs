            ...
            if let Some(tool) = tool {
                ...
                if let Some(properties) = tool.parameters.get("properties") {
                    if let Some(props_obj) = properties.as_object() {
                        ...
                        let parsed_params: Vec<Parameter> = props_obj
                            .iter()
                            .map(|(name, param)| {
                ...
                if let Ok(mock_obj) = serde_json::from_str::<serde_json::Value>(&tool.mock_response)
                {
                    if let Some(obj) = mock_obj.as_object() {
                        let parsed_fields: Vec<MockResponseField> = obj
                            .iter()
                            .map(|(name, value)| {
                                let (field_type, string_value) = match value {
                                    ...
                                    serde_json::Value::Array(_) => {
                                        ("array".to_string(), value.to_string())
                                    }
                                    serde_json::Value::Object(_) => {
                                        ("object".to_string(), value.to_string())
                                    }
                                    serde_json::Value::Null => {
                                        ("string".to_string(), "null".to_string())
                                    }
                                };
